                      ...
                      <div key={msg.id} className="border rounded-lg p-4">
                        ...
                        <div className="flex gap-2 flex-wrap">
                          ...
                          {isDelivery && msg.order && (
                            <>
                              <Select onValueChange={driverId => assignDriverMutation.mutate({ orderId: msg.order!.id, driverId: Number(driverId) })}>
                                <SelectTrigger className="w-44 h-7 text-xs"><SelectValue placeholder="Assign driver" /></SelectTrigger>
                                <SelectContent>{(allDrivers as DeliveryDriver[]).filter(d => d.status === 'AVAILABLE' && d.active).map(d => <SelectItem key={d.deliveryDriverId} value={String(d.deliveryDriverId)}>{d.firstName} {d.lastName}</SelectItem>)}</SelectContent>
                              </Select>
                              <Button
                                size="sm"
          ...
          {forwardingMessage && (
            <div className="space-y-4 py-2">
              ...
              <div className="bg-gray-50 rounded-lg p-3">
                ...
                <p className="text-xs text-gray-500 mt-0.5">{forwardingMessage.business?.businessName ?? '—'}</p>
                {forwardingMessage.order && <p className="text-xs text-duwaz-brown mt-0.5">Order #{forwardingMessage.order.id} · R{Number(forwardingMessage.order.totalAmount).toFixed(2)}</p>}
              </div>
              <p className="text-sm text-gray-600">Select a driver to forward this message to. The driver will receive full delivery details.</p>
              {/* Driver list */}
              <div className="space-y-2 max-h-64 overflow-y-auto">
                {(allDrivers as DeliveryDriver[]).filter(d => d.active).length === 0 ? (
                  ...
                  (allDrivers as DeliveryDriver[]).filter(d => d.active).map(d => {
                    ...
                    return (
                      <div key={d.deliveryDriverId} className="flex items-center justify-between border rounded-lg p-3 hover:bg-gray-50">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <Button
                            size="sm"
                            className="h-7 text-xs bg-duwaz-brown hover:bg-duwaz-brown/90"
                          ...
                          >
                            <Truck className="h-3 w-3 mr-1" />Forward
                          </Button>
                        </div>
                      </div>
                    );
